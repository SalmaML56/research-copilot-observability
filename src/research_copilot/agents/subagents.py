...
LEAD_AGENT_SYSTEM_PROMPT = (
    ...
    "error. If read_file confirms the notes file is genuinely missing, "
    "you may relaunch the researcher AT MOST ONCE MORE (two attempts "
    "total). If the file is still missing after that second attempt, "
    "STOP retrying — call finalize_report with a short report stating "
    "that research could not be completed for this topic, rather than "
    "relaunching again. Once the writer has returned the final report "
    "text to you, call the finalize_report tool with that report text as "
    "the final step — this requires human approval before it completes, "
    "so expect the run to pause there."
)
